...
```bash
npm install @nestjs/config @nestjs/swagger @nestjs/throttler nest-winston winston @prisma/client helmet zod
```

Install dev dependencies:

```bash
npm install -D prisma prettier eslint-config-prettier @eslint/js globals typescript-eslint @types/helmet
```
...
3. `app.use(helmet())` — sets secure HTTP headers (must run before any request handling)
4. `app.enableCors({ origin: cfg.frontendUrl, credentials: true })` — resolve `cfg` via `app.get(appConfig.KEY)` before calling `configureApp`, or pass it as a parameter
5. `app.use(requestIdMiddleware)` — raw Express middleware via `app.use(...)`
6. `app.useGlobalPipes(new ValidationPipe({ ... }))` — with custom `exceptionFactory`
7. `app.useGlobalInterceptors(new RequestLoggingInterceptor(...), new ResponseTransformInterceptor(...))`
8. `app.useGlobalFilters(new AppExceptionFilter(...))`
...
> **Why Helmet before request ID middleware:** Helmet sets headers on the raw response object. Placing it first means security headers are set regardless of any subsequent middleware failures. CORS before request ID ensures preflight responses include the correct headers before any app logic runs.
...
> **Why separate `message` and `errors`:** The API contract defines `message` as a human-readable summary string and `errors` as the array of field-level messages. Putting the array in `message` breaks any client that expects a string there and conflicts with the standard error envelope shape.
...
Use **Zod** to define and validate the environment schema. Do not use `class-validator` or `class-transformer` — this project uses Zod as the single validation library across all layers.

Rules:

- `PORT` — optional, coerced to number, defaults to `3000`, valid port range
- `NODE_ENV` — optional, defaults to `development`, one of `development | test | production`
- `LOG_LEVEL` — optional, defaults to `info`, one of `error | warn | info | debug | verbose`
- `LOG_FORMAT` — optional, one of `pretty | json`. Default is conditional on `NODE_ENV`:
  derive it in a `.transform()` call after the object is parsed so both fields are available:
  ```ts
  .transform((data) => ({
    ...data,
...
- `FRONTEND_URL` — optional, defaults to `http://localhost:3001` (used for CORS origin)
...
Export `Env` so config factories can type their return values against it.

> **Why Zod instead of class-validator:** This project uses Zod as the single validation library across the entire stack — environment variables, API request bodies (via shared `packages/schemas`), and frontend forms. Using class-validator only for env validation would introduce a second validation library with no benefit.

### `src/config/app.config.ts`

Expose: `port`, `nodeEnv`, `frontendUrl`.
...
Wrap every successful (non-error) response in the standard envelope:

```ts
// Single resource or action result
{ "data": <original response body>, "meta": {} }
```

Implementation notes:

- Implement `NestInterceptor`.
- Use `map` on the observable returned by `next.handle()`.
- If the controller already returns an object with a `data` key, pass it through as-is to avoid double-wrapping.
- Do not wrap `null` or `undefined` — return `{ data: null, meta: {} }` for 204-equivalent responses.
- This interceptor must be registered in `configureApp` alongside `RequestLoggingInterceptor`.

> **Why a global response transformer:** The API contract mandates `{ data, meta }` on every success response. Enforcing this globally means every controller method returns the raw payload and the shape is guaranteed by infrastructure, not by each developer remembering to wrap manually.
...
- Resolve `message` as a **string** from the exception body — if the body's `message` field is an array, join it or use a fallback string. Never put an array in `message`.
- Resolve `errors` as a `string[]` — extract from `body.errors` if present, or wrap `body.message` in an array if it is an array, or default to `[]`.
...
> **Monorepo note:** When this playbook is used inside a Turborepo monorepo (e.g. `apps/api`), place `compose.yaml` at the **monorepo root**, not inside `apps/api`. This keeps the database shared across all apps in the repo and avoids port conflicts from running multiple compose files. The `db:up`, `db:down`, and `db:reset` scripts in `apps/api/package.json` should reference the root compose file: `docker compose -f ../../compose.yaml up -d`.
...
- Use `class-validator` or `class-transformer` for env validation — use Zod
- Put an array in the `message` field of an error response — `message` must always be a string
- Skip registering `ResponseTransformInterceptor` — all success responses must use the `{ data, meta }` envelope
...
- `configureApp(...)` applies all setup steps in the specified order (including Helmet and CORS)
- `ValidationPipe` uses the custom `exceptionFactory` with string `message` and `errors[]`
- `ResponseTransformInterceptor` wraps all success responses in `{ data, meta }`
- Error responses include `errors[]`, `path`, `requestId`, `errorCode`, `timestamp`
- `getReadiness()` catches all errors and throws `ServiceUnavailableException`
