...
Implement the single onboarding completion endpoint. When called, it verifies the user has set up at least 3 categories, marks their account as onboarding-complete, and returns the updated status. This endpoint is what the frontend calls after the user finishes the category setup screen.
...
The 3-category minimum is a product decision — it ensures users have a meaningful structure before they start logging, which improves day-one activation.
...
1. Fetch the user's category count from the `categories` table
2. If count < 3, throw `BadRequestException` (400) with message: "You need at least 3 categories to complete onboarding"
...
**Check categories at completion time, not at category creation:** The 3-category minimum is enforced only when the user tries to complete onboarding, not by blocking category creation below 3. This gives users freedom to add and delete categories during onboarding without hitting an artificial constraint mid-flow.
...
- `POST /v1/onboarding/complete` with a user who has ≥ 3 categories sets `onboardingCompleted: true` and returns 200
- Same endpoint with < 3 categories returns 400
